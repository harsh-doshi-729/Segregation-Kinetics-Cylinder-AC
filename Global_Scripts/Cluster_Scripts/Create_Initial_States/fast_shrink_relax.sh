#!/bin/bash
# This script runs the shrink-relax procedure after the initial state has been relaxed by an MC run
# This script must be run after the mc_configuration file exists in a particular run folder and should be run with nohup
...
if [ -z ${numberOfMonomers} ] || [ -z ${architecture} ] || [ -z ${run} ] || [ -z ${seed} ] # -z flag checks whether the variable has zero length
then
...
long_run_script=${CREATE_INITIAL_STATES}${scriptPrefix}long_run_shrink_relax.lammps # The path to the long run LAMMPS script after shrink-relax
...
# Shrink-Relax:
${CREATE_INITIAL_STATES}Scripts/Harsh_shrink-relax -in ${shrink_relax_script} -var seed ${seed} -var numberOfMonomers ${numberOfMonomers} -var run $(printf %02d ${run}) -var angle1 ${angle1} -var angle2 ${angle2} -var finalRadius ${final_radius} -log shrink_log.lammps
error=$?
if ! [ ${error} -eq 0 ] # checking if the above process ended in an error
then
...
# Long run after shrink-relax:
${CREATE_INITIAL_STATES}Scripts/Harsh_shrink-relax -in ${long_run_script} -var seed ${seed} -var numberOfMonomers ${numberOfMonomers} -var run $(printf %02d ${run}) -var angle1 ${angle1} -var angle2 ${angle2} -var finalRadius ${final_radius} -log long_log.lammps
error=$?
if ! [ ${error} -eq 0 ] # checking if the above process ended in an error
then
        echo "The long run shrink-relax LAMMPS script did not run successfully! Terminating."
        exit ${error}
fi
