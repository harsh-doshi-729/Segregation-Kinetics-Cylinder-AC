...
if ! [ -z ${special_simulation} ]; then
	...
	mc_script=${CREATE_INITIAL_STATES}${scriptPrefix}mc_shrink_relax.lammps # The path to the Monte Carlo shrink_relax LAMMPS script
        shrink_relax_script=${CREATE_INITIAL_STATES}${scriptPrefix}shrink_relax.lammps # The path to the shrink_relax LAMMPS script 
        long_run_script=${CREATE_INITIAL_STATES}${scriptPrefix}long_run_shrink_relax.lammps # The path to the long run LAMMPS script after shrin
k-relax
...
if [ "${architecture}" != "Arc_Lin" ]; then
	...
	mpirun -np ${mc_np} ${CREATE_INITIAL_STATES}Scripts/Harsh_shrink-relax -in ${mc_script} -var seed ${seed} -var numberOfMonomers ${numberOfMonomers} -var run $(printf %02d ${run}) -var angle1 ${angle1} -var angle2 ${angle2} -var finalRadius ${final_radius} -log log.lammps
	error=$?
	if ! [ ${error} -eq 0 ] # checking if the above process ended in an error
	then
	...
	mpirun -np 4 ${CREATE_INITIAL_STATES}Scripts/Harsh_shrink-relax -in ${long_run_script} -var seed ${seed} -var numberOfMonomers ${numberOfMonomers} -var run $(printf %02d ${run}) -var angle1 ${angle1} -var angle2 ${angle2} -var finalRadius ${final_radius} -log log.lammps
	error=$?
	if ! [ ${error} -eq 0 ] # checking if the above process ended in an error
	then
