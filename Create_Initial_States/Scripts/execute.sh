...
seed=$4
special_simulation=$5 # Argument for running a particula initialization algorithm in its special folder

# filePaths:
SCRIPTS=/Global_Scripts/
CREATE_INITIAL_STATES=/Create_Initial_States/
...
then
        echo "Not enough arguments passed! Please pass number of monomers, the architecture, the run number, the two oritentation angles, the seed, and the initialization procedure folder name  while executing this script!"
...
LAMMPS_EXEC=`which lmp_gcc` # Replace the name of the LAMMPS executable for your installation if lmp_gcc is not found
# Shrink-Relax:
